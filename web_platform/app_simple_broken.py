...
def make_prediction():
    """Handle prediction requests with enhanced biomarker analysis"""
    try:
        # Get form data
        feature_values = []
        biomarker_data = {}
        ...
        for feature in FEATURES:
            ...
            if value:
                ...
                biomarker_data[feature] = float_value
            else:
        ...
        # Try to use actual trained model
        cv_risk_score = None
        ...
        if ML_AVAILABLE:
            try:
                ...
                if os.path.exists(model_path) and os.path.exists(scaler_path):
                    ...
                    # Use available features and set others to reasonable defaults
                    ...
                    print("⚠️ Model files not found, using enhanced calculation")
                    cv_risk_score = calculate_enhanced_risk_score(biomarker_data, environment)
                    model_used = "Enhanced Risk Calculator"
            except Exception as e:
                print(f"⚠️ Model loading error: {e}, using enhanced calculation")
                cv_risk_score = calculate_enhanced_risk_score(biomarker_data, environment)
                model_used = "Enhanced Risk Calculator"
        else:
            cv_risk_score = calculate_enhanced_risk_score(biomarker_data, environment)
        ...
        confidence = 0.998 if model_used == "Ridge Regression Model" else 0.85
...
def calculate_enhanced_risk_score(biomarker_data, environment):
